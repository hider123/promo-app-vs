...
// [核心修正] 引入 WithdrawalSettingsPage
...
const UserLayout = () => {
    ...
    // [核心修正] 在 renderAccountPage 的 switch 中加入 'withdrawalSettings' 的 case
    ...
    return (
        <div className="h-full flex flex-col">
            ...
            <div className="fixed top-4 right-4 z-30 flex items-center gap-3">
                <button
                    onClick={() => showAlert('您目前沒有新通知。')}
                    className="flex items-center justify-center h-12 w-12 rounded-full bg-white shadow-lg ring-2 ring-gray-200 hover:bg-gray-100 transition-colors"
                    aria-label="通知"
                >
                    <i className="fas fa-bell text-xl text-gray-600"></i>
                </button>
                <button
                    onClick={() => showAlert('客服功能正在開發中，敬請期待！')}
                    className="flex items-center justify-center h-12 w-12 rounded-full bg-white shadow-lg ring-2 ring-gray-200 hover:bg-gray-100 transition-colors"
                    aria-label="客服"
                >
                    <i className="fas fa-headset text-xl text-gray-600"></i>
                </button>
                <button
                    onClick={() => setCurrentPage('account')}
                    className={`flex items-center justify-center h-14 w-14 rounded-full bg-white shadow-lg transition-all duration-300 transform hover:scale-110 hover:shadow-xl ${currentPage === 'account' ? 'ring-4 ring-indigo-400' : 'ring-2 ring-gray-200'}`}
                    aria-label="我的帳戶"
                >
            ...
            <ConfirmationModal 
                ...
                title="確認新增帳號"
            >
                <p>每次新增帳號將從您的餘額中扣除 US${(appSettings?.catPoolPrice || 0).toFixed(2)} 的費用。</p>
